...
def load_chapter(num):
    ...
    print(f"📖 Loading chapter {num} from {path}")
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            data = json.load(f)
            print(f"✅ Chapter loaded: {data['title']}")
            return data
    print(f"❌ Chapter not found: {path}")
    return None
...
async def revelation(interaction: discord.Interaction, action: str = "start"):
    try:
        print(f"🎮 /revelation {action} by {interaction.user.name}")
        ...
        if action == "start":
            ...
            print(f"✅ Game started for {interaction.user.name}, {len(scene['choices'])} choices")
        
        elif action == "status":
            progress = get_progress(user_id)
            if not progress:
                await interaction.response.send_message("❌ No progress found", ephemeral=True)
                return
