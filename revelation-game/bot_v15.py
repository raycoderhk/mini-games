...
class StoryButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            # Send story text FIRST as plain text
            ...
            if next_scene_data.get('choices'):
                ...
                # Send as NEW message with text + buttons (not edit)
                await interaction.response.send_message(story_text, view=view)
                print(f"✅ Next: {self.next_scene}", flush=True)
            else:
                await interaction.response.send_message(story_text, view=None)
                print(f"✅ Chapter {self.chapter_num} Complete!", flush=True)
            ...
            try:
                await interaction.followup.send("⏰ Session expired! Use `/revelation continue`", ephemeral=True)
            except:
                pass
...
async def revelation(interaction: discord.Interaction, action: str = "start"):
    try:
        ...
        if action == "start":
            ...
            if not chapter:
                await interaction.response.send_message("❌ Chapter 1 script not found", ephemeral=True)
                return
            ...
            # Send story text as PLAIN TEXT (not embed!)
            ...
            if not chapter:
                await interaction.response.send_message(f"❌ Chapter {progress['chapter']} not found", ephemeral=True)
                return
            
            scene = next((s for s in chapter['scenes'] if s['id'] == progress['scene']), None)
            if not scene:
                await interaction.response.send_message("❌ Scene not found! Starting new game...", ephemeral=True)
                chapter = load_chapter(1)
                save(user_id, 1, 'scene_0', 0, 0, 0, [])
                scene = chapter['scenes'][0]
            
            stats = f"❤️ {progress['love']} | 🔍 {progress['truth']} | 🌐 {progress['afterland']}"
            story_text = f"**🌊 啓示路**\n\n{scene['text']}\n\n*{stats}*"
            
            if scene.get('choices'):
                view = View(timeout=None)
                for idx, choice in enumerate(scene['choices']):
                    btn = StoryButton(choice, user_id, chapter['chapter'], progress['scene'], idx)
                    view.add_item(btn)
                await interaction.response.send_message(story_text, view=view)
                print(f"✅ Continued at scene {progress['scene']}", flush=True)
            else:
                await interaction.response.send_message(story_text)
                print(f"✅ Chapter {progress['chapter']} Complete!", flush=True)
            ...
            embed.add_field(name="Scene", value=progress['scene'])
            ...
            embed.add_field(name="Choices", value=str(len(progress['choices'])))
...
async def testbtn(interaction: discord.Interaction):
    ...
    btn.callback = cb
    view.add_item(btn)
    await interaction.response.send_message("🔧 Test - Click the button!", view=view)
