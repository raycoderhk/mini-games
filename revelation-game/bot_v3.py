...
class ChoiceButton(Button):
    def __init__(self, choice, user_id, chapter, scores, message_id):
        super().__init__(label=choice['text'][:45], style=discord.ButtonStyle.primary,
                        emoji=choice['emoji'], custom_id=f"c_{user_id}_{message_id}_{choice['text'][:10]}")
        self.choice = choice
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if next_scene:
                ...
                embed.set_footer(text=f"❤️ {self.scores['love']} | 🔍 {self.scores['truth']} | 🌐 {self.scores['afterland']}")
                
                if next_scene['choices']:
                    # Create NEW view with NEW message_id tracking
                    view = View(timeout=600)
                    for c in next_scene['choices']:
                        view.add_item(ChoiceButton(c, self.user_id, self.chapter, self.scores, interaction.message.id))
                    
                    # Disable old buttons by editing view
                    await interaction.response.edit_message(embed=embed, view=view)
                else:
                    await interaction.response.edit_message(embed=embed, view=None)
            else:
                await interaction.response.send_message("❌ Scene not found. Use `/revelation start` to restart", ephemeral=True)
    ...
    try:
        ...
        if action == "start":
            ...
            for choice in scene['choices']:
                view.add_item(ChoiceButton(choice, user_id, chapter, scores, None))
