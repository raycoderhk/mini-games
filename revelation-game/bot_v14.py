...
class StoryButton(Button):
    def __init__(self, choice, user_id, chapter_num, current_scene, choice_idx, msg_id):
        super().__init__(
            label=choice['text'][:50],
            style=discord.ButtonStyle.primary,
            emoji=choice['emoji'],
            custom_id=f"s:{user_id}:{current_scene}:{choice_idx}:{msg_id}"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            # Send STORY TEXT first (separate message)
            story_text = f"**🌊 啓示路**\n\n{next_scene_data['text']}\n\n*{stats}*"
            await interaction.response.send_message(story_text)
            
            # Send BUTTONS second (separate message)
            if next_scene_data.get('choices'):
                view = View(timeout=None)
                for idx, c in enumerate(next_scene_data['choices']):
                    btn = StoryButton(c, self.user_id, self.chapter_num, self.next_scene, idx, self.msg_id + 1)
                    view.add_item(btn)
                
                # Send buttons as followup
                await interaction.followup.send("**請選擇：**", view=view, ephemeral=False)
...
async def revelation(interaction: discord.Interaction, action: str = "start"):
    try:
        ...
        if action == "start":
            ...
            msg = await interaction.response.send_message(story_text)
            
            # Send BUTTONS second (separate message)
            view = View(timeout=None)
            for idx, choice in enumerate(scene.get('choices', [])):
                btn = StoryButton(choice, user_id, chapter['chapter'], 'scene_0', idx, msg.id if msg else 0)
                view.add_item(btn)
...
@bot.tree.command(name="testtext", description="Test text display")
async def testtext(interaction: discord.Interaction):
...
If you can read this, your Discord client is working correctly!
...
async def testbtn(interaction: discord.Interaction):
    view = View(timeout=None)
    async def cb(i):
        await i.response.send_message("✅ Button works!", ephemeral=True)
    btn = Button(label="CLICK ME", style=discord.ButtonStyle.success, custom_id=f"t:{interaction.user.id}")
    btn.callback = cb
    view.add_item(btn)
    await interaction.response.send_message("🔧 Button Test - Click below!", view=view)
