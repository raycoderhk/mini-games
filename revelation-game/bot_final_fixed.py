...
def format_choices_text(choices):
    """Format choices as readable text in embed"""
    text = "\n**請選擇：**\n"
...
class GameView(View):
    ...
    def make_callback(self, choice):
        async def callback(interaction: discord.Interaction):
            try:
                print(f"🎮 Button: {choice['text'][:20]} by {interaction.user.name}", flush=True)
                ...
                if next_scene:
                    ...
                    # Build embed with choices text
                    desc = next_scene['text']
                    if next_scene.get('choices'):
                        desc += format_choices_text(next_scene['choices'])
                ...
                await interaction.followup.send("⏰ Use `/revelation start`", ephemeral=True)
    ...
    try:
        ...
        if action == "start":
            ...
            if scene.get('choices'):
                desc += format_choices_text(scene['choices'])
            ...
            print(f"✅ Started with {len(scene['choices'])} choices", flush=True)
...
async def on_ready():
    ...
    init_db()
    bot.add_view(GameView("", {"scenes": []}, "", {}))  # Register persistent view
    try:
