...
class StoryButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            # Send story text as PLAIN TEXT (no embed, no formatting issues)
...
@bot.tree.command(name="testtext", description="Test if you can see text")
async def testtext(interaction: discord.Interaction):
    """Test command to verify text display"""
    try:
...
*Test completed at: """ + str(discord.utils.utcnow()) + """*"""
        
        await interaction.response.send_message(test_message)
        print(f"✅ Text test sent to {interaction.user.name}", flush=True)
    except Exception as e:
...
async def testbtn(interaction: discord.Interaction):
    ...
    btn.callback = cb
    view.add_item(btn)
    await interaction.response.send_message("🔧 Button Test - Click the button below!", view=view)
