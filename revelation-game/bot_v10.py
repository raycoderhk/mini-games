...
class StoryButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            # Send story text FIRST as separate message
            stats = f"❤️ {new_love} | 🔍 {new_truth} | 🌐 {new_afterland}"
            
            # Create clean embed with ONLY story text
            embed = discord.Embed(
                title="🌊 啓示路",
                description=next_scene_data['text'],
...
async def revelation(interaction: discord.Interaction, action: str = "start"):
    try:
        ...
        if action == "start":
            ...
            # Create embed with ONLY story text (no choices in description)
            embed = discord.Embed(
                title="🌊 啓示路：樂土之門",
                description=scene['text'],  # ← ONLY text, no choices appended!
                color=discord.Color.purple()
            )
