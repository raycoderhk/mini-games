...
class GameView(View):
    def __init__(self, user_id, chapter, scene_id, scores, timeout=600):
        ...
        if scene and scene.get('choices'):
            for i, choice in enumerate(scene['choices']):
                btn = Button(
                    ...
                    emoji=choice['emoji'],
                    custom_id=f"game_{user_id}_{scene_id}_{i}"
                )
                btn.callback = self.make_callback(choice, scene_id)
                self.add_item(btn)
    
    def make_callback(self, choice, current_scene_id):
        async def callback(interaction: discord.Interaction):
            # Verify user
            if str(interaction.user.id) != self.user_id:
                await interaction.response.send_message(
                    "❌ This is not your game! Use `/revelation start`", 
                    ephemeral=True
                )
                return
            ...
            try:
                ...
                if next_scene:
                    ...
                    await interaction.response.send_message(
                        "❌ Scene not found. Use `/revelation start` to restart",
                        ephemeral=True
                    )
                    
            except discord.errors.NotFound:
                await interaction.followup.send(
                    "⏰ Session expired! Use `/revelation start` to restart",
                    ephemeral=True
                )
            except Exception as e:
                print(f"❌ Button error: {e}")
                await interaction.followup.send(
                    f"❌ Error: {str(e)}\nUse `/revelation start` to restart",
                    ephemeral=True
                )
...
@bot.tree.command(name="revelation_continue", description="Continue your game")
